...
//Given a graph G=(V,E), create a compatible graph G'=(V,E') such that {i,j} \in E' if and only if num_VB_paths[i][j] >= r or {i,j}\in E  in G.
Graph Graph::CreateCompatipleGraph_Hereditary(){
    Graph g(nverts);
    g.nedges = 0;
    g.param_r = param_r;
    g.param_s = param_s;
    for (long i = 0; i<nverts; i++)
    {
        ...
        {
            if (num_VB_paths[i][j] >= param_r || IsAdjTo(i, j))
            {
