...
public:
    ...
    Graph CreateCompatipleGraph_Hereditary(); //Given a graph G=(V,E), create a compatible graph G'=(V,E') such that {i,j} \in E' if and only if num_VB_paths[i][j] >= param_r for {i,j}\not in E or {i,j}\in E  in G.
