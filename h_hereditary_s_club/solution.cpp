...
//Find heuristic for hereditary s-club
vector<long> findHeuristic_hereditary(Graph & input_graph){
    //find a maximal r-robust s-clique
    Graph gc = input_graph.CreateCompatipleGraph_Hereditary(); //create a compatible graph
    ...
    //check if S is a r-hereditary s-club; if not, remove vertices until remaining vertices from a r-hereditary s-club
    bool flag = true;
    while (flag) {
        flag = false;
        ...
        if (!IsHereditary_sClub(tau,induceG)) {
...
//check if input_graph is r-Hereditary s-club
bool IsHereditary_sClub(vector<long> &tau, Graph & input_graph){
    bool isHereditary = true;
...
//Do vertex peeling based on Algorithm 6 in the paper
void vertexPeeling_Hereditary(long LB, Graph & input_graph){
    ...
    while (flag){
        ...
        if (!isCaculated || iterations >=2)
            FindVBPathsByRhoUBLB_Hereditary(input_graph);
        for (long i1=0; i1<n; i1++) {
            ...
            if (sizeKNbrs <=LB-1) {
                ...
                for (long t=0; t<sizeKNbrs; t++) {
                    ...
                    if (input_graph.IsAdjTo(i1,i2) || (!input_graph.IsAdjTo(i1,i2) && input_graph.num_VB_paths[i1][i2] <= input_graph.param_r-1)){
                        tmpVal = tmpVal - 1;
                        //if |Wv| + |N(v)| <LB, then remove all incident edges with v
                        if (tmpVal + input_graph.degree[i1]  <= LB-1) {
                            for (long j1=0; j1<input_graph.degree[i1]; j1++) {
                                long a = input_graph.AdjList[i1][j1];
                                if (IsInVector(input_graph.AdjList[a],i1)) {
                                    RemoveVectorItem(input_graph.AdjList[a],i1);
